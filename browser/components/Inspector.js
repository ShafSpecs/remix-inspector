...
`;
export function RemixInspector({ defaultOpen = false, draggable = true, position = "top-right" }) {
    ...
    return (_jsxs(Disclosure, { defaultOpen: defaultOpen, as: "div", id: "remix-inspector", children: [_jsx("style", { dangerouslySetInnerHTML: { __html: STYLES } }), _jsx(Disclosure.Button, { className: "disclosure-button", ref: element, id: draggable ? "buttonheader" : "", children: _jsx(CodeIcon, { "aria-hidden": true, className: "icon-code" }) }), _jsx(Disclosure.Panel, { className: "disclosure-panel", ref: rightPanel, unmount: false, children: _jsxs("section", { className: "disclosure-panel-section", children: [_jsxs(Header, { children: [_jsx("h2", { children: "Remix Inspector" }), _jsx(Disclosure.Button, { id: "play-button", children: _jsx(CloseIcon, { "aria-label": "Close", className: "icon-close" }) })] }), _jsxs("div", { children: [_jsxs("section", { className: "routes-section", children: [_jsxs(Header, { children: [_jsx("h2", { children: "Routes" }), _jsx("button", { type: "button", className: "refresh", onClick: () => refresh(), children: "Refresh" }), state === "pending" ? (_jsx(RefreshIcon, { "aria-label": "Pending", className: "icon-refresh" })) : null] }), _jsx("ul", { children: matches.map((match) => {
                                                return (_jsx("li", { children: _jsx("button", { className: clsx("route-option", {
                                                            "route-option-active": match.id === activeMatchId,
                                                        }), type: "button", onClick: () => setActiveMatchId(match.id), children: match.id }) }, match.id));
                                            }) })] }), _jsxs("section", { className: "route-details-section", children: [_jsx(Header, { children: _jsx("h2", { style: { lineHeight: "20px" }, children: "Route Details" }) }), _jsx("div", { style: { paddingTop: 8, paddingBottom: 8 }, children: _jsx(Table, { data: [
                                                    { key: "Pathname", value: activeMatch.pathname },
                                                    { key: "Route ID", value: activeMatch.id },
                                                ] }) }), _jsx(Explorer, { data: activeMatch.data, heading: "Data Explorer" }), _jsx(Explorer, { data: activeMatch.params, heading: "Params Explorer" }), _jsx(Explorer, { data: activeMatch.handle, heading: "Handle Explorer" })] })] })] }) })] }));
}
function Header({ children }) {
    return _jsx("header", { className: "header", children: children });
}
function Explorer({ data, heading }) {
    return (_jsxs(_Fragment, { children: [_jsx(Header, { children: _jsx("h3", { children: heading }) }), _jsx("div", { className: "json-tree", children: _jsx(JSONTree, { data: data, invertTheme: false, theme: {
                        scheme: "monokai",
...
function Table({ data }) {
    return (_jsx("table", { style: { width: "100%" }, children: _jsx("tbody", { children: data.map((row, index) => {
                return (_jsxs("tr", { className: "space-y-1", children: [_jsx("td", { style: {
                                paddingLeft: 8,
                                paddingRight: 8,
                                width: "25%",
                            }, className: "text-slate-300", children: row.key }), _jsx("td", { children: row.value })] }, index));
            }) }) }));
}
